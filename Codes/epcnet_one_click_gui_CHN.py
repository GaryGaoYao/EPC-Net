...
def check_nifti(path: str, label: str) -> None:
    ...
    if not path.lower().endswith((".nii", ".nii.gz")):
        raise ValueError(f"{label}必须是 .nii 或 .nii.gz 文件。")
...
def normalize_stl_path(path: str) -> str:
    if not path:
        raise ValueError("请选择最终 STL 的保存路径。")
    if not path.lower().endswith(".stl"):
        path += ".stl"
    parent = os.path.dirname(os.path.abspath(path))
    if not parent:
        raise ValueError("STL 保存路径无效。")
...
def normalize_state_dict(obj: Any) -> Dict[str, torch.Tensor]:
    ...
    if not isinstance(obj, dict):
        raise ValueError("模型文件不是有效的 PyTorch state_dict/checkpoint。")
    ...
    if not state:
        raise ValueError("模型文件中未找到可加载的网络权重。")
    return state
...
def save_meta_tensor_as_nifti(
    ...
    if array.ndim == 4 and array.shape[0] == 1:
        array = array[0]
    if array.ndim != 3:
        raise ValueError(f"预测 mask 维度错误：{array.shape}")
...
def infer_eye(
    ...
    if not mask_has_foreground(output_path):
        raise RuntimeError("Stage 1 未检测到眼球区域。请检查 CT 和眼球模型。")
    return output_path
...
def infer_bone(
    ...
    if not mask_has_foreground(output_path):
        raise RuntimeError("Stage 2 未检测到眶骨区域。请检查 CT 和骨模型。")
    return output_path
...
def sitk_scalar_to_vtk_image(image: sitk.Image) -> vtkImageData:
    ...
    if not np.any(array_zyx > 0):
        raise RuntimeError("骨分割为空，无法生成 STL。")
...
def reconstruct_stl(mask_path: str, output_path: str) -> str:
    ...
    if mesh is None or mesh.GetNumberOfPoints() == 0:
        raise RuntimeError("高斯平滑后未能生成有效网格。")
...
class PipelineWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            check_nifti(self.ct_path, "输入 CT")
            check_weight(self.stage1_weight, "Stage 1 模型")
            check_weight(self.stage2_weight, "Stage 2 模型")
            ...
            with tempfile.TemporaryDirectory(prefix="epcnet_") as temp_dir:
                ...
                self.progress_changed.emit(5, f"正在准备（{device_name}）…")
                self.progress_changed.emit(12, "正在定位眼球…")
                infer_eye(
                ...
                self.progress_changed.emit(45, "正在分割眶骨…")
                infer_bone(
                    ct_path=self.ct_path,
                    eye_path=eye_path,
                ...
                self.progress_changed.emit(82, "正在自动平滑并生成 STL…")
...
class FileRow:
    ...
    def __init__(self, label_text: str, button_text: str) -> None:
        ...
        self.edit.setPlaceholderText("未选择")
...
class ModelSettingsDialog(QDialog):
    ...
    def __init__(
        ...
        self.stage1_field = FileRow("Stage 1 眼球模型", "选择")
        self.stage2_field = FileRow("Stage 2 眶骨模型", "选择")
        ...
        self.stage1_field.button.clicked.connect(
            lambda: self._select_weight(self.stage1_field, "选择 Stage 1 权重")
        )
        self.stage2_field.button.clicked.connect(
            lambda: self._select_weight(self.stage2_field, "选择 Stage 2 权重")
        )
    ...
    def _validate_and_accept(self) -> None:
        try:
            check_weight(self.stage1_field.text(), "Stage 1 模型")
            check_weight(self.stage2_field.text(), "Stage 2 模型")
        except Exception as exc:  # noqa: BLE001
            QMessageBox.warning(self, "模型路径无效", str(exc))
            return
        self.accept()
...
class EPCNetSTLWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.ct_field = FileRow("CT / CBCT NIfTI", "选择")
        self.stl_field = FileRow("STL 保存位置", "另存为")
        ...
        self.model_status = QLabel("模型未设置")
        ...
        self.run_button = QPushButton("生成 STL")
    ...
    def _update_model_status(self) -> None:
        ...
        self.model_status.setText("模型已就绪" if ready else "请设置模型")
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        self.run_button.setText("正在生成…" if busy else "生成 STL")

    def _start(self) -> None:
        try:
            check_nifti(self.ct_field.text(), "输入 CT")
            check_weight(self.stage1_weight_path, "Stage 1 模型")
            check_weight(self.stage2_weight_path, "Stage 2 模型")
    ...
    def _completed(self, stl_path: str) -> None:
        ...
        QMessageBox.information(self, "完成", f"STL 已保存：\n\n{stl_path}")
    ...
    def closeEvent(self, event) -> None:  # type: ignore[override]
        if self.worker is not None and self.worker.isRunning():
            QMessageBox.information(self, "处理中", "请等待当前处理完成后再关闭。")
            event.ignore()
            return
        event.accept()
